...
public class AdminController implements Initializable {
    ...
    ObservableList<String> GroupschooseJoin = FXCollections.observableArrayList();
...
Label UpdaterCreateGroup;
    public void CreateGroup() throws Exception {
        ...
        groups.addGroup(new GroupModel(NewGroup.getText()));

GroupschooseJoin.clear();
GroupChoiceJoin.getItems().clear();

groups.Populate(GroupschooseJoin);
        GroupChoiceJoin.setItems(GroupschooseJoin);
        groups.save();
        GroupChoiceJoin.setValue(UserInitialGroup);

        UpdaterCreateGroup.setTextFill(Color.web("#00FF00", 1));
        UpdaterCreateGroup.setText("Group " + NewGroup.getText() + " created!");


    }

    @FXML
    TextField DeleteGroup;
    @FXML
    Label UpdaterDeleteGroup;
public void DeleteGroup() throws Exception {
    String Username = UserName.getText();
    String Email = UsernameFunc.getEmailByUsername(Username);
    String UserInitialGroup = group.getInitialGroupByEmail(Email);

    groups.DeleteGroup(DeleteGroup.getText());
    GroupschooseJoin.clear();
    GroupChoiceJoin.getItems().clear();
    groups.Populate(GroupschooseJoin);
       groups.save();
GroupChoiceJoin.setValue(UserInitialGroup);
    ...
    UpdaterDeleteGroup.setTextFill(Color.web("#FFFF00", 1));
    UpdaterDeleteGroup.setText("Attempt in removing " + DeleteGroup.getText() + " group..");


}

    @FXML
    TextField ExistingGroup;
    @FXML
    TextField UpdatedGroup;
    @FXML
    Label UpdaterGroup;
...
public void UpdateGroup() throws Exception {
    ...
    groups.UpdateGroup(ExistingGroup.getText(),UpdatedGroup.getText());
    GroupschooseJoin.clear();
    GroupChoiceJoin.getItems().clear();
    groups.Populate(GroupschooseJoin);
    groups.save();
    GroupChoiceJoin.setValue(UserInitialGroup);
    UpdaterGroup.setTextFill(Color.web("#FFFF00", 1));
    UpdaterGroup.setText("Attempt in updating " + ExistingGroup.getText() + " group to " + UpdatedGroup.getText());
...
String Info = "";
    ...
    for(int i = 0; i < List.size();i++) {
        if (GroupChoiceFilter.getSelectionModel().getSelectedItem().toString().equals(List.get(i).getGroup())) {


            Info = Info + "Email:  ";
            Info = Info + UsernameFunc.getEmailByUsername(List.get(i).getUsername());
            Info = Info+ " User: ";
            Info = Info + List.get(i).getUsername();
            Info = Info + "  Group:  ";
            Info = Info + List.get(i).getGroup();
            Info = Info + "\n\n";
        }
...
public void resetLabels(){
    UpdaterDeleteGroup.setText("");
    UpdaterCreateGroup.setText("");
    ErrorCheckerTitle.setText("");
UpdaterGroup.setText("");
}

@FXML
TextField UsertoRemove;
...
try {
    ...
    String UsertoEmail = UsernameFunc.getEmailByUsername(UsertoRemove.getText());
//Removing from saved data
    group.RemoveGroup(UsertoEmail);
    LoggingFunc.RemoveLogging(UsertoEmail);
    UsernameFunc.RemoveUsername(UsertoEmail);
    UserTypeFunc.RemoveType(UsertoEmail);
    //Saving removed progress
    group.save();
    LoggingFunc.save();
    UsernameFunc.save();
    UserTypeFunc.save();
//Removing from Table
    for (int i = 0; i < List.size(); i++) {
        ...
        if (List.get(i).getUsername().equals(UsertoRemove.getText())) {
            System.out.println(List.size() + List.get(i).getUsername());
            List.remove(i);
